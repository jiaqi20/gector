...
class Seq2Labels(Model):
    ...
    def forward(self,  # type: ignore
        ...
        logits_labels = self.tag_labels_projection_layer(self.predictor_dropout(encoded_text)).type(torch.long)[:,:,0]
        logits_d = self.tag_detect_projection_layer(encoded_text).type(torch.long)[:,:,1]

        logits_labels = torch.transpose(logits_labels, 0, 1)
        logits_d = torch.transpose(logits_d, 0, 1)
        ...
        emissions_l = torch.randn(sequence_length, batch_size, self.num_labels_classes)
        class_probabilities_labels = crf_label(emissions_l, logits_labels)

        # class_probabilities_d = F.log_softmax(logits_d, dim=-1).view(
        #     [batch_size, sequence_length, self.num_detect_classes])
        emissions_d = torch.randn(sequence_length, batch_size, self.num_detect_classes)
        class_probabilities_d = crf_d(emissions_d, logits_d)
